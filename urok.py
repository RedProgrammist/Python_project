...
while programm_work:
    ...
    if not const_lim:
        quest = input(f"""Do you want a {lim} line query length limit?\n If yes, enter "yes" or anything,\n If you want to change the limit, enter any positive number,\n If you don't want a limit, enter a negative number\n""")
        if quest.lstrip('-+').isdigit():
            quest = int(lim)

        if inst_const_lim:
            inst_const_lim = False
            quest = input("""If you want to keep the limit for all subsequent requests, enter "yes". Attention! Your choice can only be changed by turning off the program!\n""")
            if quest.lower() == "yes":
                const_lim = True
    fetch = []
    if req_num <= 2:
        if lim >=0:
            cursor.execute(reqs[int(req_num) - 1].func(*inp) + f" Limit {lim}")
        else:
            cursor.execute(reqs[int(req_num) - 1].func(*inp))

        fetch = cursor.fetchall()
        cursor_save.execute("CREATE TABLE IF NOT EXISTS requests (request_id INT AUTO_INCREMENT PRIMARY KEY,req_type VARCHAR(35) NOT NULL,arg1 VARCHAR(100),arg2 INT,arg3 INT,cnt_req INT,req_date date);")
        cnt_req(cursor_save, reqs[int(req_num) - 1].name, inp)

    else:
        if lim >= 0:
            cursor.execute("select * from requests" + f" Limit {lim}")
        else:
